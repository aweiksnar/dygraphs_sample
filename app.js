...
(function() {
  var CanvasState, Events, Shape, canvasState, graph, item, iterator, normal_data, num, super_data, _i, _j, _len;
  ...
  Events = {
    ...
    onDoubleClick: function(event, g, context) {
      ...
      if (this.zoomed) {
        _ref = g.xAxisExtremes(), minX = _ref[0], maxX = _ref[1];
      } else {
        minX = g.toDataXCoord(event.layerX);
        maxX = minX + g.xAxisExtremes()[1] / 5;
      }
  ...
  for (num = _i = 1; _i <= 10; num = ++_i) {
    for (_j = 0, _len = light_curve_data.length; _j < _len; _j++) {
      item = light_curve_data[_j];
      super_data.push([item['x'] + iterator, item['y']]);
    }
    iterator += 35;
  }

  graph = new Dygraph(document.getElementById("graph"), super_data, {
    drawPoints: true,
    drawXGrid: true,
    drawYGrid: false,
    drawAxesAtZero: true,
